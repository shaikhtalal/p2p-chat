...
func NewChatUI(cr *chatroom.ChatRoom) *ChatUI {
	...
	nodeID := withColor("yellow", fmt.Sprintf("%s", cr.ConnectToNodeID))
	// make a text view to contain our chat messages
	msgBox := tview.NewTextView()
	msgBox.SetDynamicColors(true)
	msgBox.SetBorder(true)
	// msgBox.SetTitle(fmt.Sprintf("Room: %s", cr.RoomName))
	msgBox.SetTitle(fmt.Sprintf(" %s %s ", nodeID, "Replace 127.0.0.1 with Public IP"))
